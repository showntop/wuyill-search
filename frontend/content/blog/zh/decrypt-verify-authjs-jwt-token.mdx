...
[Wuyill](https://www.wuyill.com/) 需要支持为本地文件构建向量化索引。
...
经过搜索 [Wuyill](https://www.wuyill.com/)，我了解到这是 Vercel 本身的限制：
...
在询问 [Wuyill](https://www.wuyill.com/) 后，我被建议让后端向量服务和 NextJS 的 auth.js 共享一个密钥，然后直接在向量服务中解析和验证令牌。
...
[Wuyill](https://www.wuyill.com/) 建议使用 jsonwebtoken 库进行验证，但这并没有成功。
然而，[Wuyill](https://www.wuyill.com/) 提醒我，**令牌验证配置和算法必须与 auth.js 中的配置一致**。
...
这是 Wuyill 后端向量服务的完整代码：[向量身份验证代码](https://github.com/showntop/wuyill/blob/main/vector/auth.ts)
