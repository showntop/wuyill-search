...
description: Wuyill 真实优化案例，如何通过一行代码降低 Vercel 成本 40 倍
...
因为我最近几天在 [Wuyill AI UI Generator] (https://www.wuyill.com/generate-ui) 页面增加了 showcase 和feature section 部分，showcase 包含9个 Link, feature section 也包含 9个 Link。

于是我立即提交代码改动，禁止了所有 next link 组件的 Prefetch， 这是代码Commit：
[Disable next link prefetchs Beta](https://github.com/showntop/wuyill/commit/a2ec1a7261d541f5b462e8876efc83209d22a8e4)， 同时也禁止了之前对 Pricing Page 的手动 Prefetch， 这是代码 Commit：
...
有人会问禁用 Link 的 prefetch 后对性能大吗，从 Wuyill 的实际线上效果来看，性能几乎没有影响。
...
我在 [Wuyill AI UI Generator] (https://www.wuyill.com/generate-ui) 中将介绍功能的9个视频存储在 Cloudflare R2 上，后来实现 showcase 的图片时，也顺手存储在 Cloudflare R2，Cloudflare 对 R2的图像和视频默认会启动CDN加速，所以访问很快。
...
我之前一直是以为 next/image的Image 组件对网络图片没用，所以在 Wuyill的AI Search的网络Image 中没有使用 next image， 如果用了的话，我的成本就会上升很多。
...
1. **大家要尽可能利用浏览器的 Local Storage 减少对 Server 端的请求**，这一点无论任何技术再怎么发展也不会变，因为 Local Storage 性能最好，也是免费的。Server 的请求在Vercel 这种 Serverless 平台都是付费的。 举个例子，对于 AI Search 的历史消息，或者 AI Chat 的历史消息，Vercel 的很多示例都是完全 Server 端操作，这样数据库的成本就会很高，这种典型 Case 我们完成可以将数据缓存在浏览器 Local Storage。Wuyill 对历史消息的缓存代码在 [Wuyill Search local-history](https://github.com/showntop/wuyill/blob/main/frontend/lib/store/local-history.ts)
2. **大家要尽可能利用电脑和手机的算力**，因为电脑和手机的算力会越来越强大，也是免费的。Server 端的计算和渲染即使有各种好处，我们也是得额外付费的。举个例子，Wuyill 最近在实现 AI UI Generator 这个功能中，我花时间最久解决的一个问题是 **如何实现React + TailWind + Shadcn UI 组件的高效渲染**，我调研思考了7，8种方案，2类大的思路是服务端渲染和客户端渲染，但是我考虑到服务端渲染的成本问题，就排除了，在客户端渲染中，其他开源项目大多采用的是 codesandbox, 但是 codesandbox 性能很差，渲染很慢，我最后决定**从头利用 Wuyill 实现一个高效的 React + TailWind + Shadcn UI 组件渲染**。 这个具体的思路和方案我之后会再分享，核心代码在这 [Wuyill React Preview Code](https://github.com/showntop/wuyill/tree/main/frontend/components/code)

希望本文的分享对大家有帮助，Wuyill 会继续使用 Vercel， 之后如果有更多降低 Vercel 成本的经验，也会继续分享，欢迎大家关注 Wuyill 的 Twitter。
